#include <newt/protocol/steem_operations.hpp>

#include <newt/chain/block_summary_object.hpp>
#include <newt/chain/compound.hpp>
#include <newt/chain/custom_operation_interpreter.hpp>
#include <newt/chain/database.hpp>
#include <newt/chain/database_exceptions.hpp>
#include <newt/chain/db_with.hpp>
#include <newt/chain/evaluator_registry.hpp>
#include <newt/chain/global_property_object.hpp>
#include <newt/chain/history_object.hpp>
#include <newt/chain/index.hpp>
#include <newt/chain/steem_evaluator.hpp>
#include <newt/chain/steem_objects.hpp>
#include <newt/chain/transaction_object.hpp>
#include <newt/chain/shared_db_merkle.hpp>
#include <newt/chain/operation_notification.hpp>
#include <newt/chain/witness_schedule.hpp>

#include <newt/chain/util/asset.hpp>
#include <newt/chain/util/reward.hpp>
#include <newt/chain/util/uint256.hpp>
#include <newt/chain/util/reward.hpp>
...
FC_REFLECT( newt::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( newt::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( newt::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
void database::reindex( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t shared_file_size )
{
   try
   {
      ...
      newt_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{ try {
   if( !has_hardfork( newt_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( newt_HARDFORK_0_17__769 ) || comment.parent_author == newt_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( newt_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(newt_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //newt_MAX_BLOCK_SIZE;
   ...
   if( has_hardfork( newt_HARDFORK_0_5__54 ) )
   {
      const auto& witness = get_witness( witness_owner );

      if( witness.running_version != newt_BLOCKCHAIN_VERSION )
         pending_block.extensions.insert( block_header_extensions( newt_BLOCKCHAIN_VERSION ) );

      const auto& hfp = get_hardfork_property_object();

      if( hfp.current_hardfork_version < newt_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == newt_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > newt_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= newt_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      newt_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   newt_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   newt_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   newt_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   newt_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   newt_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   newt_TRY_NOTIFY( on_applied_transaction, tx )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = newt_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / newt_BLOCK_INTERVAL + 1;
}
...
std::pair< asset, asset > database::create_sbd( const account_object& to_account, asset steem, bool to_reward_balance )
{
   ...
   try
   {
      ...
      {
         auto to_sbd = ( gpo.sbd_print_rate * steem.amount ) / newt_100_PERCENT;
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( newt_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, newt_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != newt_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= newt_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = newt_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = newt_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( newt_HARDFORK_0_2 ) )
      ...
      if( has_hardfork( newt_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( newt_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( newt_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( newt_NULL_ACCOUNT );
...
{
   if( head_block_num() >= newt_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / newt_100_PERCENT ).to_uint64();
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( newt_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_vesting( voter, asset( claim, STEEM_SYMBOL ), has_hardfork( newt_HARDFORK_0_17__659 ) );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( newt_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / newt_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / newt_100_PERCENT;
               auto vest_created = create_vesting( get_account( b.account ), benefactor_tokens, has_hardfork( newt_HARDFORK_0_17__659 ) );
            ...
            auto sbd_steem     = ( author_tokens * comment.percent_steem_dollars ) / ( 2 * newt_100_PERCENT ) ;
            auto vesting_steem = author_tokens - sbd_steem;

            const auto& author = get_account( comment.author );
            auto vest_created = create_vesting( author, vesting_steem, has_hardfork( newt_HARDFORK_0_17__659 ) );
            auto sbd_payout = create_sbd( author, sbd_steem, has_hardfork( newt_HARDFORK_0_17__659 ) );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( newt_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == newt_ROOT_POST_PARENT )
         {
            if( has_hardfork( newt_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + newt_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( newt_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( newt_FIRST_CASHOUT_TIME ) )
      return;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( newt_HARDFORK_0_19__1051 ) )
            decay_rate = newt_RECENT_RSHARES_DECAY_RATE_HF19;
         else
            decay_rate = newt_RECENT_RSHARES_DECAY_RATE_HF17;
   ...
   if( has_hardfork( newt_HARDFORK_0_17__771 ) )
   {
...
void database::process_funds()
{
   ...
   if( has_hardfork( newt_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( newt_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / newt_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( newt_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_steem = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( newt_100_PERCENT ) * int64_t( newt_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_steem * newt_CONTENT_REWARD_PERCENT ) / newt_100_PERCENT;
      if( has_hardfork( newt_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
      auto vesting_reward = ( new_steem * newt_VESTING_FUND_PERCENT ) / newt_100_PERCENT; /// 15% to vesting fund
      auto witness_reward = new_steem - content_reward - vesting_reward; /// Remaining 10% to witness pay

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= newt_MAX_WITNESSES;
   ...
   else
   {
      ...
      if( props.head_block_number < newt_START_VESTING_BLOCK )
...
{
   if( has_hardfork( newt_HARDFORK_0_12__178 ) )
      return asset( 0, STEEM_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( newt_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< newt_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL );
   return std::max( percent, newt_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( newt_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< newt_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL );
   return std::max( percent, newt_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( newt_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< newt_CURATE_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL);
   return std::max( percent, newt_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( newt_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< newt_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL);
   auto pay = std::max( percent, newt_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= newt_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) ) {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least newt_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < newt_MAX_WITNESSES && props.head_block_number < newt_START_VESTING_BLOCK )
      return asset( 0, STEEM_SYMBOL );
#endif

   static_assert( newt_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( newt_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< newt_POW_APR_PERCENT >( props.virtual_supply.amount ), STEEM_SYMBOL);
   return std::max( percent, newt_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % newt_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( newt_HARDFORK_0_8__116 ) )
      return newt_1_PERCENT * 25;
   else
      return newt_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / newt_100_PERCENT;
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + newt_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, newt_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < newt_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = newt_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(newt_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = newt_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = newt_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = newt_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = newt_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = newt_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = newt_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < newt_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = newt_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = newt_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = newt_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = newt_INIT_MINER_NAME;
         p.time = newt_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, STEEM_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = newt_MAX_BLOCK_SIZE;
      } );
      ...
      {
         hpo.processed_hardforks.push_back( newt_GENESIS_TIME );
      } );

      // Create witness scheduler
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         wso.current_shuffled_witnesses[0] = newt_INIT_MINER_NAME;
      } );
   }
   FC_CAPTURE_AND_RETHROW()
}
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      newt_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         newt_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
{ try {
   ...
   if( has_hardfork( newt_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < newt_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",newt_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( newt_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
try {
   if( (head_block_num() % newt_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( newt_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_sbd_exchange_update + newt_MAX_FEED_AGE_SECONDS
            && !wit.sbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.sbd_exchange_rate );
         }
      }
      else if( wit.last_sbd_exchange_update < now + newt_MAX_FEED_AGE_SECONDS &&
          !wit.sbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.sbd_exchange_rate );
      }
   ...
   if( feeds.size() >= newt_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t newt_feed_history_window = newt_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( newt_HARDFORK_0_16__551) )
            newt_feed_history_window = newt_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > newt_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( newt_HARDFORK_0_14__230 ) )
            {
               const auto& gpo = get_dynamic_global_properties();
...
{ try {
   ...
   const chain_id_type& chain_id = newt_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, newt_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         newt_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      newt_ASSERT( trx.expiration <= now + fc::seconds(newt_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",newt_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( newt_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         newt_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      newt_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( newt_HARDFORK_0_14__278 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > newt_BLOCKS_PER_DAY )
                  {
   ...
   {
      newt_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < newt_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",newt_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      ...
      if( !median_price.is_null() && has_hardfork( newt_HARDFORK_0_14__230 ) )
      {
         auto percent_sbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_sbd_supply * get_feed_history().current_median_history ).amount.value ) * newt_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_sbd <= newt_SBD_START_PERCENT )
            dgp.sbd_print_rate = newt_100_PERCENT;
         else if( percent_sbd >= newt_SBD_STOP_PERCENT )
            dgp.sbd_print_rate = 0;
         else
            dgp.sbd_print_rate = ( ( newt_SBD_STOP_PERCENT - percent_sbd ) * newt_100_PERCENT ) / ( newt_SBD_STOP_PERCENT - newt_SBD_START_PERCENT );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < newt_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > newt_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - newt_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( newt_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((newt_100_PERCENT - newt_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / newt_100_PERCENT);
...
int database::match( const limit_order_object& new_order, const limit_order_object& old_order, const price& match_price )
{
   ...
   if( !has_hardfork( newt_HARDFORK_0_12__178 ) &&
       ( (age >= newt_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( newt_HARDFORK_0_10__149)) ||
       (age >= newt_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( newt_HARDFORK_0_10__149) ) ) )
   {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= newt_LIQUIDITY_TIMEOUT_SEC )
         {
            r.sbd_volume = 0;
            r.steem_volume = 0;
            r.weight = 0;
         ...
         r.update_weight( has_hardfork( newt_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( newt_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
         ...
         case SBD_SYMBOL:
            if( a.sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.sbd_seconds > 0 &&
                   (acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment).to_seconds() > newt_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.sbd_seconds / newt_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= newt_100_PERCENT;
         ...
         case SBD_SYMBOL:
            if( a.savings_sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_sbd_seconds > 0 &&
                   (acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment).to_seconds() > newt_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_sbd_seconds / newt_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= newt_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), SBD_SYMBOL);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < newt_BLOCKS_PER_DAY*7 )
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( newt_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( newt_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_1 ] = fc::time_point_sec( newt_HARDFORK_0_1_TIME );
   _hardfork_versions[ newt_HARDFORK_0_1 ] = newt_HARDFORK_0_1_VERSION;
   FC_ASSERT( newt_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_2 ] = fc::time_point_sec( newt_HARDFORK_0_2_TIME );
   _hardfork_versions[ newt_HARDFORK_0_2 ] = newt_HARDFORK_0_2_VERSION;
   FC_ASSERT( newt_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_3 ] = fc::time_point_sec( newt_HARDFORK_0_3_TIME );
   _hardfork_versions[ newt_HARDFORK_0_3 ] = newt_HARDFORK_0_3_VERSION;
   FC_ASSERT( newt_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_4 ] = fc::time_point_sec( newt_HARDFORK_0_4_TIME );
   _hardfork_versions[ newt_HARDFORK_0_4 ] = newt_HARDFORK_0_4_VERSION;
   FC_ASSERT( newt_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_5 ] = fc::time_point_sec( newt_HARDFORK_0_5_TIME );
   _hardfork_versions[ newt_HARDFORK_0_5 ] = newt_HARDFORK_0_5_VERSION;
   FC_ASSERT( newt_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_6 ] = fc::time_point_sec( newt_HARDFORK_0_6_TIME );
   _hardfork_versions[ newt_HARDFORK_0_6 ] = newt_HARDFORK_0_6_VERSION;
   FC_ASSERT( newt_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_7 ] = fc::time_point_sec( newt_HARDFORK_0_7_TIME );
   _hardfork_versions[ newt_HARDFORK_0_7 ] = newt_HARDFORK_0_7_VERSION;
   FC_ASSERT( newt_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_8 ] = fc::time_point_sec( newt_HARDFORK_0_8_TIME );
   _hardfork_versions[ newt_HARDFORK_0_8 ] = newt_HARDFORK_0_8_VERSION;
   FC_ASSERT( newt_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_9 ] = fc::time_point_sec( newt_HARDFORK_0_9_TIME );
   _hardfork_versions[ newt_HARDFORK_0_9 ] = newt_HARDFORK_0_9_VERSION;
   FC_ASSERT( newt_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_10 ] = fc::time_point_sec( newt_HARDFORK_0_10_TIME );
   _hardfork_versions[ newt_HARDFORK_0_10 ] = newt_HARDFORK_0_10_VERSION;
   FC_ASSERT( newt_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_11 ] = fc::time_point_sec( newt_HARDFORK_0_11_TIME );
   _hardfork_versions[ newt_HARDFORK_0_11 ] = newt_HARDFORK_0_11_VERSION;
   FC_ASSERT( newt_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_12 ] = fc::time_point_sec( newt_HARDFORK_0_12_TIME );
   _hardfork_versions[ newt_HARDFORK_0_12 ] = newt_HARDFORK_0_12_VERSION;
   FC_ASSERT( newt_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_13 ] = fc::time_point_sec( newt_HARDFORK_0_13_TIME );
   _hardfork_versions[ newt_HARDFORK_0_13 ] = newt_HARDFORK_0_13_VERSION;
   FC_ASSERT( newt_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_14 ] = fc::time_point_sec( newt_HARDFORK_0_14_TIME );
   _hardfork_versions[ newt_HARDFORK_0_14 ] = newt_HARDFORK_0_14_VERSION;
   FC_ASSERT( newt_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_15 ] = fc::time_point_sec( newt_HARDFORK_0_15_TIME );
   _hardfork_versions[ newt_HARDFORK_0_15 ] = newt_HARDFORK_0_15_VERSION;
   FC_ASSERT( newt_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_16 ] = fc::time_point_sec( newt_HARDFORK_0_16_TIME );
   _hardfork_versions[ newt_HARDFORK_0_16 ] = newt_HARDFORK_0_16_VERSION;
   FC_ASSERT( newt_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_17 ] = fc::time_point_sec( newt_HARDFORK_0_17_TIME );
   _hardfork_versions[ newt_HARDFORK_0_17 ] = newt_HARDFORK_0_17_VERSION;
   FC_ASSERT( newt_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_18 ] = fc::time_point_sec( newt_HARDFORK_0_18_TIME );
   _hardfork_versions[ newt_HARDFORK_0_18 ] = newt_HARDFORK_0_18_VERSION;
   FC_ASSERT( newt_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ newt_HARDFORK_0_19 ] = fc::time_point_sec( newt_HARDFORK_0_19_TIME );
   _hardfork_versions[ newt_HARDFORK_0_19 ] = newt_HARDFORK_0_19_VERSION;


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= newt_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("newt_NUM_HARDFORKS",newt_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= newt_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( newt_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ newt_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( newt_HARDFORK_0_5__54 ) )
      {
         ...
         {
            if( hardforks.last_hardfork < newt_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < newt_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < newt_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= newt_NUM_HARDFORKS; i++ )
   {
      if( i <= newt_HARDFORK_0_5__54 )
   ...
   switch( hardfork )
   {
      case newt_HARDFORK_0_1:
...
#ifdef IS_TEST_NET
         {
            ...
            test_op.required_auths.insert( newt_INIT_MINER_NAME );
...
#endif
         break;
      case newt_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case newt_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case newt_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case newt_HARDFORK_0_5:
         break;
      case newt_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case newt_HARDFORK_0_7:
         break;
      case newt_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case newt_HARDFORK_0_9:
         {
         ...
         break;
      case newt_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case newt_HARDFORK_0_11:
         break;
      case newt_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == newt_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + newt_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + newt_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( newt_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( newt_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( newt_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case newt_HARDFORK_0_13:
         break;
      case newt_HARDFORK_0_14:
         break;
      case newt_HARDFORK_0_15:
         break;
      case newt_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > newt_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case newt_HARDFORK_0_17:
         {
            static_assert(
               newt_MAX_VOTED_WITNESSES_HF0 + newt_MAX_MINER_WITNESSES_HF0 + newt_MAX_RUNNER_WITNESSES_HF0 == newt_MAX_WITNESSES,
               "HF0 witness counts must add up to newt_MAX_WITNESSES" );
            static_assert(
               newt_MAX_VOTED_WITNESSES_HF17 + newt_MAX_MINER_WITNESSES_HF17 + newt_MAX_RUNNER_WITNESSES_HF17 == newt_MAX_WITNESSES,
               "HF17 witness counts must add up to newt_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = newt_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = newt_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = newt_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = newt_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = newt_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = newt_1_PERCENT * 25;
               rfo.percent_content_rewards = newt_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_steem;
#ifndef IS_TEST_NET
               rfo.recent_claims = newt_HF_17_RECENT_CLAIMS;
#endif
            ...
            */
            ...
            root_posts.reserve( newt_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( newt_HF_17_NUM_REPLIES );
            ...
            for( auto itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + newt_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( auto itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + newt_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case newt_HARDFORK_0_18:
         break;
      case newt_HARDFORK_0_19:
         {
            ...
            modify( get< reward_fund_object, by_name >( newt_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = newt_HF_19_RECENT_CLAIMS;
#endif
...
void database::validate_invariants()const
{
   try
   {
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_vsf_votes += ( itr->proxy == newt_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( newt_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[newt_MAX_PROXY_RECURSION_DEPTH - 1] :
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / newt_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < newt_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != newt_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != newt_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != newt_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.id, witness_id_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.id )
