...
int main(int argc, char** argv) {
    ...
    if(dataPath.length() != 0 && !fc::exists(dataPath)){
        fc::create_directories(dataPath);
    }
    if(dataPath.length() > 0){
        dataPath = "testnet";
        steemit::configManager::getInstance(dataPath);
    }
   ...
   try {
...
#ifdef IS_TEST_NET
      std::cerr << "------------------------------------------------------\n\n";
      std::cerr << "            STARTING NEWS NETWORK\n\n";
      std::cerr << "------------------------------------------------------\n";
      ...
      std::cerr << "------------------------------------------------------\n";
       std::cerr << NEWS_PRIVATE_EQUITY <<"\n";
//       std::cerr << graphene::utilities::key_to_wif(NEWS_PRIVATE_EQUITY_PRIVATE_KEY) <<"\n";
       std::cerr << NEWS_EARLY_INVESTMENT <<"\n";
//       std::cerr << graphene::utilities::key_to_wif(NEWS_EARLY_PRIVATE_KEY) <<"\n";
       std::cerr << NEWS_TRAFFIC_MARKET <<"\n";
//       std::cerr << graphene::utilities::key_to_wif(NEWS_TRAFFIC_PRIVATE_KEY) <<"\n";
       std::cerr << NEWS_CORE_TEAM <<"\n";
//       std::cerr << graphene::utilities::key_to_wif(NEWS_CORE_PRIVATE_KEY) <<"\n";
       std::cerr << NEWS_FOUNDATION <<"\n";
//       std::cerr << graphene::utilities::key_to_wif(NEWS_FOUNDATION_PRIVATE_KEY) <<"\n";
#else
      std::cerr << "------------------------------------------------------\n\n";
