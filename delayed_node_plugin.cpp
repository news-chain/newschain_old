...
struct delayed_node_plugin_impl {
   ...
   fc::api<newt::app::database_api> database_api;
   boost::signals2::scoped_connection client_connection_closed;
   newt::chain::block_id_type last_received_remote_head;
   newt::chain::block_id_type last_processed_remote_head;
};
}
...
void delayed_node_plugin::connect()
{
   ...
   my->database_api = my->client_connection->get_remote_api<newt::app::database_api>(0);
...
void delayed_node_plugin::sync_with_trusted_node()
{
   ...
   while( true )
   {
      newt::chain::dynamic_global_property_object remote_dpo = my->database_api->get_dynamic_global_properties();
      ...
      {
         fc::optional<newt::chain::signed_block> block = my->database_api->get_block( db.head_block_num()+1 );
...
newt_DEFINE_PLUGIN( delayed_node, newt::delayed_node::delayed_node_plugin )
