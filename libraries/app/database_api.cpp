 ...
 */
        vector<set<string>> database_api_impl::get_key_references(vector<public_key_type> keys) const {
            FC_ASSERT(false,
                      "database_api::get_key_references has been deprecated. Please use account_by_key_api::get_key_references instead.");
...
//////////////////////////////////////////////////////////////////////
        ...
        vector<extended_account> database_api_impl::get_accounts(vector<string> names) const {
            ...
            for (auto name: names) {
                ...
                if (itr != idx.end()) {
                    auto cc = extended_account(*itr, _db);
                    cc.level.updateLevel();
   ...
   return result;*/
        ...
        optional<account_bandwidth_api_obj>
        database_api::get_account_bandwidth(string account, witness::bandwidth_type type) const {
...
//   wdump((result));
        ...
        bool database_api_impl::verify_authority(const signed_transaction &trx) const {
            trx.verify_authority(STEEMIT_CHAIN_ID,
                                 ...
                                 [&](string account_name) {
                                     return authority(
                                             _db.get<account_authority_object, by_account>(account_name).owner);
                                 },
                                 [&](string account_name) {
                                     return authority(
        ...
        vector<discussion>
        database_api::get_content_replies(string author, string permlink, string beginLink, uint32_t limit) const {
            return my->_db.with_read_lock([&]() {
                ...
                if (beginLink == "") {
                    ...
                    bool flag = false;
                    for (uint32_t i = 0; itr != by_permlink_idx.end(); itr++) {
                        if (i == limit) {
                            break;
                        }
                        if (to_string(itr->permlink) == beginLink && !flag) {
                            flag = true;
                        }
                        if (itr->parent_author == author && to_string(itr->parent_permlink) == permlink && flag) {
                            i++;
                            result.push_back(discussion(*itr));
                            set_pending_payout(result.back());
                        }
 ...
 */
        vector<discussion>
        database_api::get_replies_by_last_update(account_name_type start_parent_author, string start_permlink,
        ...
        database_api::get_account_history(string account, uint64_t from, uint32_t limit) const {
            return my->_db.with_read_lock([&]() {
                FC_ASSERT(limit <= 10000, "Limit of ${l} is greater than maxmimum allowed", ("l", limit));
                FC_ASSERT(from >= limit, "From must be greater than limit");
                FC_ASSERT(from != 0, "from != 0");
...
//            return get_account_history_By_date(account, from, limit);
        }

        map<uint32_t, applied_operation>
        database_api::get_account_history_by_date(string account, uint32_t dateBegin, uint32_t dateEnd) const {
            return my->_db.with_read_lock([&]() {
                FC_ASSERT(dateBegin < dateEnd, "dateBegin must < dateEnd");
                const auto &idx = my->_db.get_index<account_history_index>().indices().get<by_account>();
                auto itr = idx.lower_bound(boost::make_tuple(account, -1));
                //   if( itr != idx.end() ) idump((*itr));

                auto end = idx.upper_bound(boost::make_tuple(account, 0));

                fc::time_point_sec timeStart(dateBegin);
                fc::time_point_sec timeEnd(dateEnd);

                map<uint32_t, applied_operation> result;
                while (itr != end) {
//                    uint32_t se = itr->sequence;
                    auto ret = my->_db.get(itr->op);
                    if (ret.timestamp + (60 * 60 * 24 * 2) < timeStart) {
                        break;
                    }
                    if (ret.timestamp < timeStart || ret.timestamp > timeEnd) {
                        itr++;
                        continue;
                    }
//                    steemit::protocol::operation op = ret;
//                    if(ret.id == steemit::protocol::operation){
//
//                    }

                    result[itr->sequence] = ret;
                    itr++;
                }
            ...
            while (count > 0 && tidx_itr != tidx.end()) {
                ...
                if (itr_count > max_itr_count) {
                    ...
                    wlog("count=${count}   itr_count=${itr_count}   filter_count=${filter_count}   exc_count=${exc_count}",
                         ("count", count)("itr_count", itr_count)("filter_count", filter_count)("exc_count",
                                                                                                exc_count));
                    break;
                }
 ...
 */
        void database_api::recursively_fetch_content(state &_state, discussion &root,
                                                     set<string> &referenced_accounts) const {
            return my->_db.with_read_lock([&]() {
                try {
                    if (root.author.size())
        ...
        //add
        vector<read_history> database_api::get_account_read_history(string account, uint32_t dateBegin, uint32_t dateEnd) const {
            return my->_db.with_read_lock([&](){
                vector<read_history> result;
                const auto &idx = my->_db.get_index<comment_read_index>().indices().get<by_reader_comment>();
                const auto &account_idx = my->_db.get_index<account_index>().indices().get<by_name>();
                auto user = account_idx.find(account);

                const auto &commentIdx = my->_db.get_index<comment_index>().indices().get<by_id>();
                if(dateEnd == 0){
                    dateEnd = UINT32_MAX;
                }
                for(auto itr = idx.begin(); itr != idx.end(); itr++){
                    if(itr->reader == user->id){
                        if(itr->first_read.sec_since_epoch() > dateBegin && itr->first_read.sec_since_epoch() < dateEnd){
                            auto comment = commentIdx.find(itr->comment);
                            read_history r;
                            r.reader = user->name;
                            r.author = comment->author;
                            r.permlink = to_string(comment->permlink);
                            r.first_time = itr->first_read;
                            r.last_time = itr->last_read;
        ...
        vector<share_history> database_api::get_account_share_history(string account, uint32_t dateBegin, uint32_t dateEnd) const {
            return my->_db.with_read_lock([&](){
                vector<share_history> result;

                const auto &shareIndex = my->_db.get_index<comment_share_index>().indices().get<by_id>();
                const auto &account_idx = my->_db.get_index<account_index>().indices().get<by_name>();
                ...
                for(auto itr = shareIndex.begin(); itr != shareIndex.end(); itr++){
                    if(itr->sharer == user->id){
                        if(itr->first_share.sec_since_epoch() > dateBegin && itr->first_share.sec_since_epoch() < dateEnd){
                            auto comment = commentIdx.find(itr->comment);
                            share_history s;
                            s.sharer = user->name;
                            s.author = comment->author;
                            s.permlink = to_string(comment->permlink);
                            s.first_time = itr->first_share;
                            s.last_time = itr->last_share;
                            result.push_back(s);
                        }
        ...
        vector<other_asset> database_api::get_other_asset(uint32_t page, uint32_t num) {
            return my->_db.with_read_lock([&](){
                vector<other_asset> result;
//                const  auto &itr_asset = my->_db.get_index<asset_symbol_object_index>().indices().get<by_id>();
//                const auto &begin = itr_asset.begin();
//                uint32_t beginNum = page * num;
//                for(auto itr = begin; itr != itr_asset.end(); itr++){
//                    if(beginNum > 0){
//                        beginNum--;
//                        continue;
//                    }
...
//
//                    other_asset asset1;
//                    asset1.creator = itr->create_account_name;
//                    asset1.tag_name = itr->tag_name;
//                    asset1.create_time = itr->create_time;
//                    asset1.symbol = itr->create_symbol_type;
//                    asset1.amount = itr->amount;
//                    result.push_back(asset1);
//                }
                return result;
            });
        }
        ...
        map<string, vector<simple_other_asset>> database_api::get_accounts_asset(vector<string> accounts) {
            return my->_db.with_read_lock([&](){
                map<string, vector<simple_other_asset>> result;
//                const auto &itrIndex = my->_db.get_index<account_asset_index>().indices().get<by_asset_account_name>();
//                for(auto itr = accounts.begin(); itr != accounts.end(); itr++){
//                    const auto &asset_itr = itrIndex.find(*itr);
//                    if(asset_itr == itrIndex.end()){
////                        FC_ASSERT(false, "cant not find accounts ${n}", ("n", *itr));
//                        result[*itr] = std::vector<simple_other_asset>();
//                        continue;
//                    }
//                    vector<simple_other_asset> vv;
//                    for(auto itrAssert = asset_itr->asset_array.begin(); itrAssert != asset_itr->asset_array.end();itrAssert++){
//                        simple_other_asset simple;
////                        simple.name = itrAssert->tag_name;
////                        simple.amount = itrAssert->amount;
//                        simple.symbol = itrAssert->symbol;
//                        vv.push_back(simple);
//                    }
//                    result[*itr] = vv;
//                }
        ...
        vector<reward_detail_history> database_api::get_reward_detail_history(uint64_t begin, uint64_t limit) {
            FC_ASSERT(limit <= 1000, "limit must <= 1000");

            return my->_db.with_read_lock([&](){

                vector<reward_detail_history> result;
                const auto &idx_itr = my->_db.get_index<reward_detail_index>().indices().get<by_id>();
                if(begin == -1){
                    begin = INT64_MAX;
                }
                const auto &up_itr = idx_itr.upper_bound(begin);
//                const auto &begin = idx_itr.lower_bound(limit);
                for(auto itr = up_itr; itr != idx_itr.begin();){
                    itr--;
                    reward_detail_history history;
                    history.operation = to_string(itr->op);
                    history.creator = to_string(itr->creator);
                    history.amount = itr->amount;
                    history.created_time = itr->created_time;
                    history.weight = itr->weight;
                    history.to_user = to_string(itr->to_user);
                    history.index = itr->id._id;
        ...
        vector<reward_detail_relationship> database_api::get_detail_relationship(int64_t begin,
                                                                                 int64_t limit) {
            FC_ASSERT(limit <= 1000, "limit must <= 1000");
            return my->_db.with_read_lock([&](){
                vector<reward_detail_relationship> result;
                const auto &idx_itr = my->_db.get_index<reward_for_teacher_index>().indices().get<by_id>();
                const auto &start = idx_itr.lower_bound(begin);
                for(auto itr = start; itr != idx_itr.end(); itr++){
                    reward_detail_relationship relationship;
                    relationship.user = to_string(itr->user);
                    relationship.parent = to_string(itr->parent);
                    relationship.teacher = to_string(itr->teacher);
                    relationship.parent_amount = itr->parent_amount;
                    relationship.teacher_amount = itr->teacher_amount;
                    relationship.create_time = itr->created_time;
                    relationship.user_amount = itr->user_amount;
                    relationship.op = to_string(itr->op);
                    relationship.index = itr->id._id;
        ...
        string database_api::get_dynamic_global_pool() {
            return my->_db.with_read_lock([&](){
                const auto &pool = my->_db.get_dynamic_global_pool_object();
                std::map<std::string, uint64_t > vv;
                vv["author_pool"] = pool.author_pool;
                vv["cost_author"] = pool.cost_author;

                vv["reader_pool"] = pool.reader_pool;
                vv["cost_reader"] = pool.cost_reader;

                vv["share_pool"] = pool.other_pool;
                vv["cost_share"] = pool.cost_other;

                return fc::json::to_string(vv);
            });
        }



        vector<transfer_object> database_api::get_transfer_by_index(int64_t begin, int32_t limit) {
            FC_ASSERT(limit <= 1000 && limit > 0, "limit = (0,1000]");
            return my->_db.with_read_lock([&](){
                const auto &tf_index = my->_db.get_index<transfer_history_index>().indices().get<by_id>();
                vector<transfer_object> result;

                auto start = tf_index.lower_bound(begin);
                for(auto itr = start; itr != tf_index.end(); itr++){
                    transfer_object obj;
                    obj.from =  (itr->from_name);
                    obj.to = (itr->to_name);
                    obj.amount = itr->amount.amount.value;
                    obj.type = itr->amount.symbol;
                    obj.memo = to_string(itr->memo);
                    obj.created_time = itr->created_time;
                    obj.index = itr->id._id;
        ...
        vector<transfer_object> database_api::get_transfer_by_from_to(string from, string to, int64_t page, int64_t limit){
            FC_ASSERT(limit <= 1000 && limit > 0, "limit = (0,1000]");
            FC_ASSERT(from.size() != 0 || to.size() != 0, "");
            FC_ASSERT(page > 0, "page must > 0");
            return my->_db.with_read_lock([&](){
                vector<transfer_object> result;
                const auto &tf_index = my->_db.get_index<transfer_history_index>().indices().get<by_from_to>();
                auto start = tf_index.lower_bound(boost::make_tuple(from, to));
                auto end = tf_index.upper_bound(boost::make_tuple(from, to));
                ...
                for(auto itr = start; itr != end && itr != tf_index.end(); itr++){
                    if((string)itr->from_name == from && (string)itr->to_name == to){
                        transfer_object obj;
                        obj.from =  (itr->from_name);
                        obj.to = (itr->to_name);
                        obj.amount = itr->amount.amount.value;
        ...
        vector<transfer_object> database_api::get_transfer_by_account(string account, int64_t page, int64_t limit){
            FC_ASSERT(limit <= 1000 && limit > 0, "limit = (0,1000]");
            FC_ASSERT(account.size() > 0, "");
            return my->_db.with_read_lock([&](){
                vector<transfer_object> result;
                const auto &tf_from_idx = my->_db.get_index<transfer_history_index>().indices().get<by_from>();
                const auto &tf_to_idx = my->_db.get_index<transfer_history_index>().indices().get<by_to>();

                auto from_lower = tf_from_idx.lower_bound(account);
                auto from_upper = tf_from_idx.upper_bound(account);

                auto to_lower = tf_to_idx.lower_bound(account);
                auto to_upper = tf_to_idx.upper_bound(account);

                vector<transfer_object> temp;
                for(auto itr = from_lower; itr != from_upper; itr++){
                    if(itr->from_name == account){
                        transfer_object obj;
                        obj.from =  (itr->from_name);
                        obj.to = (itr->to_name);
                        obj.amount = itr->amount.amount.value;
                ...
                for(auto itr = to_lower; itr != to_upper; itr++){
                    if(itr->to_name == account){
                        transfer_object obj;
                        obj.from =  (itr->from_name);
                        obj.to = (itr->to_name);
                        obj.amount = itr->amount.amount.value;
                ...
                std::sort(temp.begin(), temp.end(), [](const transfer_object &o1, const transfer_object &o2){
                    return o1.index < o2.index;
                });

                if(temp.size() < (page - 1) * limit){
                    return result;
                }

                for(int64_t i = (page - 1) * limit; i < temp.size(); i++){
        ...
        vector<transfer_object> database_api::get_transfer_by_memo(string memo) {
            return my->_db.with_read_lock([&](){
                vector<transfer_object> result;
                const auto &memo_idx = my->_db.get_index<transfer_history_index>().indices().get<by_memo>();
//                shared_string ff(memo.begin(), memo.end());
//                from_string(ff, memo);
                auto begin = memo_idx.lower_bound(memo);
                auto end = memo_idx.upper_bound(memo);
                for(auto itr = begin; itr != end && itr != memo_idx.end(); itr++){
                    if(to_string(itr->memo) == memo){
                        transfer_object obj;
                        obj.from =  (itr->from_name);
                        obj.to = (itr->to_name);
                        obj.amount = itr->amount.amount.value;
