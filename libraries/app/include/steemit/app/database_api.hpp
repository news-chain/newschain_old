 ...
 */
        ...
        public:
            ...
            get_account_bandwidth(string account, witness::bandwidth_type type) const;
            ...
            get_vesting_delegations(string account, string from, uint32_t limit = 100) const;
            ...
            vector<discussion>
            get_content_replies(string parent, string parent_permlink, string beginlink = "", uint32_t limit = 0) const;
             ...
             */
            vector<discussion>
            get_discussions_by_author_before_date(string author, string start_permlink, time_point_sec before_date,
            ...
            get_account_history_by_date(string account, uint32_t dateBegin, uint32_t dateEnd) const;


            /*
             * @param account : account name
             * @param dateBegin: begin Time;
             * @param dateEnd: end time
             * if end time == 0 , dateEnd = dafult now time;
             * */
            vector<read_history> get_account_read_history(string account, uint32_t dateBegin, uint32_t dateEnd) const;


            /*
             *
             * */
            vector<share_history> get_account_share_history(string account, uint32_t dateBegin, uint32_t dateEnd) const;

            /*
             * get system all symbol
             * */
            vector<other_asset> get_other_asset(uint32_t page, uint32_t num);



            /*
             * get susers all symbol
             * */
            map<string, std::vector<simple_other_asset>> get_accounts_asset(vector<string> accounts);


            vector<reward_detail_history> get_reward_detail_history(uint64_t begin, uint64_t limit);
            vector<reward_detail_relationship> get_detail_relationship(int64_t begin, int64_t limit);



            //transfer_history
            vector<transfer_object> get_transfer_by_index(int64_t begin, int32_t limit);
            vector<transfer_object> get_transfer_by_from_to(string from, string to, int64_t page, int64_t limit);
            vector<transfer_object> get_transfer_by_account(string account, int64_t page, int64_t limit);
            vector<transfer_object> get_transfer_by_memo(string memo);
            ////////////////////////////
            // Handlers - not exposed //
            ////////////////////////////
            void on_api_startup();

            string get_dynamic_global_pool();
        ...
        private:
            ...
            vector<discussion> get_discussions(const discussion_query &q,
                                               ...
                                               const std::function<bool(
                                                       const comment_api_obj &)> &filter = &database_api::filter_default,
                                               const std::function<bool(
                                                       const comment_api_obj &)> &exit = &database_api::exit_default,
                                               const std::function<bool(
                                                       const tags::tag_object &)> &tag_exit = &database_api::tag_exit_default,
...
FC_REFLECT(steemit::app::discussion_query,
           (tag)(filter_tags)(select_tags)(select_authors)(truncate_body)(start_author)(start_permlink)(parent_author)(
                   parent_permlink)(limit));
...
// Subscriptions
       (set_block_applied_callback)
               ...
               (get_reward_detail_history)
               (get_detail_relationship)
               ...
               (get_account_read_history)
               (get_account_share_history)
               (get_other_asset)
               (get_accounts_asset)
               // Market
               (get_order_book)
               ...
               (get_miner_queue)
                ...
                //transfer history
               (get_transfer_by_index)
               (get_transfer_by_from_to)
               (get_transfer_by_account)
               (get_transfer_by_memo)
)
