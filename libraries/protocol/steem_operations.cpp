...
namespace steemit {
    namespace protocol {
        ...
        void account_create_operation::validate() const {
            ...
            FC_ASSERT(is_asset_type(fee, STEEM_SYMBOL) || is_asset_type(fee, NEWS_SYMBOL), "Account creation fee must be STEEM or NEWST");
            owner.validate();
            active.validate();
        ...
        void comment_operation::validate() const {
            ...
            if(extends.size() == 0 || extends.size() > 64){
                FC_ASSERT(false, "extends.size() > 64 || extends.size() == 0");
            }
        }
        ...
        void read_comment_operation::validate() const {
        ...
        void share_comment_operation::validate() const {
...
//                FC_ASSERT(from != to, "cant transfer to yourself");
        ...
        void
        equihash_pow::create(const block_id_type &recent_block, const account_name_type &account_name, uint32_t nonce) {
            input.worker_account = account_name;
        ...
        void feed_publish_operation::validate() const {
            ...
            FC_ASSERT(
                    (is_asset_type(exchange_rate.base, STEEM_SYMBOL) && is_asset_type(exchange_rate.quote, SBD_SYMBOL))
                    ||
        ...
        void claim_reward_balance_operation::validate() const {
            ...
            FC_ASSERT(reward_steem.amount > 0 || reward_sbd.amount > 0 || reward_vests.amount > 0,
        ...
        void vote_up_operation::validate() const {
            validate_account_name(voter);
            validate_account_name(author);
            validate_permlink(permlink);
            if(extends.size() == 0 || extends.size() > 64){
        ...
        void vote_down_operation::validate() const {
            validate_account_name(voter);
            validate_account_name(author);
            validate_permlink(permlink);

//            FC_ASSERT(extends.size() > 0 && extends.size() < 64, " extends must <= 64!");
            if( extends.size() > 64){
                FC_ASSERT(false, "extends.size() > 64 ");
            }
        }
        ...
        void create_asset_operation::validate() const{
            validate_account_name(creator);
            if(tag_name.size() == 0){
                FC_ASSERT(false, "tag_name.size == 0");
            }
        }

       void transfer_other_asset_operation::validate() const {
           validate_account_name(from);
       }
