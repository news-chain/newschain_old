...
#include <steemit/chain/util/reward.hpp>
#include <steemit/chain/reward_detail.hpp>
...
namespace steemit {
    namespace chain {
        ...
        void database::open(const fc::path &data_dir, const fc::path &shared_mem_dir, uint64_t initial_supply,
 ...
 */
        ...
        database::create_sbd(const account_object &to_account, asset steem, bool to_reward_balance) {
 ...
 */
       ...
       *adjust_balancemultiplying of 64 bit numbers. This is done in asset and price.
       */
                asset new_vesting = steem * (to_reward_balance ? cprops.get_reward_vesting_share_price()
                    ...
                    while (itr != cvidx.end() && itr->comment == c.id) {
                        ...
                        if (claim > 0) // min_amt is non-zero satoshis
                        {
                            ...
                            auto reward = create_vesting(voter, asset(claim, STEEM_SYMBOL),
                                                         has_hardfork(STEEMIT_HARDFORK_0_17__659));
...
//                        auto sbd_payout = create_sbd(author, sbd_steem, has_hardfork(STEEMIT_HARDFORK_0_17__659));
                        auto sbd_payout = create_sbd(author, sbd_steem, false);

                        adjust_total_payout(comment, sbd_payout.first +
                                                     to_sbd(sbd_payout.second + asset(vesting_steem, STEEM_SYMBOL)),
...
//                return;
            ...
            if (has_hardfork(STEEMIT_HARDFORK_0_17__771)) {
                while (current != cidx.end() && current->cashout_time <= head_block_time()) {
                    if (current->net_rshares > 0) {
                        ...
                        funds[rf.id._id].recent_claims += util::evaluate_reward_curve(current->net_rshares.value,
    ...
    */
            auto com = *current;
            bool timeBool = current->cashout_time <= head_block_time();
            while (current != cidx.end() && timeBool) {
                if (has_hardfork(STEEMIT_HARDFORK_0_17__771)) {
 ...
 */
       ...
       */
                ...
                const auto &producer_reward = create_vesting(get_account(cwit.owner),
        ...
        asset database::get_content_reward() const {
            ...
            asset percent(
                    protocol::calc_percent_reward_per_block<STEEMIT_CONTENT_APR_PERCENT>(props.virtual_supply.amount),
                    STEEM_SYMBOL);
        ...
        asset database::get_curation_reward() const {
            ...
            static_assert( STEEMIT_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
            asset percent(
                    protocol::calc_percent_reward_per_block<STEEMIT_CURATE_APR_PERCENT>(props.virtual_supply.amount),
                    STEEM_SYMBOL);
...
//            static_assert(STEEMIT_MAX_WITNESSES == 21, "this code assumes 21 per round");
            asset percent(calc_percent_reward_per_round<STEEMIT_POW_APR_PERCENT>(props.virtual_supply.amount),
                          STEEM_SYMBOL);
 ...
 */
        ...
        void database::initialize_evaluators() {
            ...
            _my->_evaluator_registry.register_evaluator<read_comment_evaluator>(); //add by zhang on 2018.2.27
            _my->_evaluator_registry.register_evaluator<share_comment_evaluator>(); //add by zhang on 2018.3.1
            ...
            _my->_evaluator_registry.register_evaluator<vote_up_evaluator>();
            _my->_evaluator_registry.register_evaluator<vote_down_evaluator>();
            _my->_evaluator_registry.register_evaluator<create_asset_evaluator>();
            _my->_evaluator_registry.register_evaluator<transfer_other_asset_evaluator>();
        ...
        void database::initialize_indexes() {
            ...
            add_core_index<dynamic_global_pool_index>(*this);
            ...
            add_core_index<comment_read_index>(*this);
            add_core_index<comment_share_index>(*this);
            add_core_index<comment_vote_up_index>(*this);
            add_core_index<comment_vote_down_index>(*this);
            ...
            add_core_index<account_asset_index>(*this);
            add_core_index<asset_symbol_object_index>(*this);
            add_core_index<reward_detail_index>(*this);
            add_core_index<reward_for_teacher_index>(*this);
            add_core_index<transfer_history_index>(*this);
        ...
        void database::init_genesis(uint64_t init_supply) {
            try {
                struct auth_inhibitor {
                    auth_inhibitor(database &db) : db(db), old_flags(
                            db.node_properties().skip_flags) { db.node_properties().skip_flags |= skip_authority_check; }
...
//                        wlog("create minter name : ${n}", ("n", a.name));
                        a.balance = asset(i ? 0 : init_supply, NEWS_SYMBOL);
//                        a.balance = asset(123456, STEEM_SYMBOL);
                ...
                /*create 5 account**/
                auto names = {NEWS_PRIVATE_EQUITY, NEWS_EARLY_INVESTMENT, NEWS_TRAFFIC_MARKET, NEWS_CORE_TEAM,
                              NEWS_FOUNDATION};
                auto amount = {2000000000000ll, 1000000000000ll, 1500000000000ll, 1000000000000ll, 1000000000000ll};
                auto publicKey = {NEWS_PRIVATE_EQUITY_PUBLIC_KEY, NEWS_EARLY_PUBLIC_KEY, NEWS_TRAFFIC_PUBLIC_KEY,
                                  NEWS_CORE_PUBLIC_KEY, NEWS_FOUNDATION_PUBLIC_KEY};

                auto itr_amount = amount.begin();
                auto itr_public_key = publicKey.begin();
                for (auto itr = names.begin(); itr != names.end(); itr++) {
                    create<account_object>([&](account_object &obj) {
                        obj.name = *itr;
                        obj.balance.amount = *itr_amount;
                        obj.memo_key = *itr_public_key;

                    });

                    create<account_authority_object>([&](account_authority_object &auth) {
                        auth.account = *itr;
                        auth.owner.add_authority(*itr_public_key, 1);
                        auth.owner.weight_threshold = 1;
                        auth.active = auth.owner;
                        auth.posting = auth.active;
                    });
...
//                    });
                ...
                create<dynamic_global_pool_object>([&](dynamic_global_pool_object &obj) {

                });

                // Nothing to do
                create<feed_history_object>([&](feed_history_object &o) {
                    o.current_median_history.base = asset(1000);
                    o.current_median_history.quote = asset(800, SBD_SYMBOL);
                });
            ...
            try {
                ...
                if (BOOST_LIKELY(head_block_num() > 0)) {
                    if (!(skip & skip_tapos_check)) {
                        ...
                        STEEMIT_ASSERT(trx.ref_block_prefix == tapos_block_summary.block_id._hash[1],
                    ...
                    if (has_hardfork(
                        ...
                        STEEMIT_ASSERT(now < trx.expiration, transaction_expiration_exception, "",
...
//                ilog("witness_object ${w}", ("w", (std::string)witness.owner));
                if (!(skip & skip_witness_signature))
                ...
                if (!(skip & skip_witness_schedule_check)) {
                    ...
                    string scheduled_witness = get_scheduled_witness(slot_num);
//                    ilog("witness_object ${w}", ("w", (std::string)scheduled_witness));
...
//            for(int i = 0; i < 20; i++){
//                ilog("_hardfork_versions :${u}", ("u", _hardfork_versions[i]));
//            }
        ...
        void database::apply_hardfork(uint32_t hardfork) {
            ...
            switch (hardfork) {
                ...
                case STEEMIT_HARDFORK_0_17: {
                    static_assert(
                            STEEMIT_MAX_VOTED_WITNESSES_HF0 + STEEMIT_MAX_MINER_WITNESSES_HF0 +
                            STEEMIT_MAX_RUNNER_WITNESSES_HF0 == STEEMIT_MAX_WITNESSES,
                            "HF0 witness counts must add up to STEEMIT_MAX_WITNESSES");
                    static_assert(
                            STEEMIT_MAX_VOTED_WITNESSES_HF17 + STEEMIT_MAX_MINER_WITNESSES_HF17 +
                            STEEMIT_MAX_RUNNER_WITNESSES_HF17 == STEEMIT_MAX_WITNESSES,
...
//                asset total_supply = asset(0, NEWS_SYMBOL);
//                asset total_spro  bd = asset(0, SBD_SYMBOL);
...
//
//                total_supply +=
//                        gpo.total_vesting_fund_steem + gpo.total_reward_fund_steem + gpo.pending_rewarded_vesting_steem;
...
//                FC_ASSERT(gpo.pending_rewarded_vesting_steem == pending_vesting_steem, "",
//                          ("pending_rewarded_vesting_steem", gpo.pending_rewarded_vesting_steem)(
//                                  "pending_vesting_steem", pending_vesting_steem));
//
            ...
            try {
                ...
                for (const auto &account : get_index<account_index>().indices()) {
                    modify(account, [&](account_object &a) {
                        ...
                        a.vesting_withdraw_rate = asset(a.to_withdraw / STEEMIT_VESTING_WITHDRAW_INTERVALS_PRE_HF_16,
                                                        VESTS_SYMBOL);
...
// Low memory nodes only need immediate child count, full nodes track total children
        ...
        const dynamic_global_pool_object &database::get_dynamic_global_pool_object() const {
            try {
                return get<dynamic_global_pool_object>();
            } FC_CAPTURE_AND_RETHROW()
        }

        void database::production_new_coin() {
            const auto &pool = get_dynamic_global_pool_object();

            //48.000
            uint64_t _0_1_percent = 48;
            uint32_t step = 10;
            modify(pool, [&](dynamic_global_pool_object &obj) {
                obj.curation_pool += _0_1_percent * STEEMIT_CURATION_PERCENT / step;
                obj.initminer_pool += _0_1_percent * STEEMIT_INITMINER_PERCENT / step;
                obj.author_pool += _0_1_percent * STEEMIT_AUTHOR_PERCENT / step;
                obj.pow_pool += _0_1_percent * STEEMIT_POW_PERCENT / step;
                obj.other_pool += _0_1_percent * STEEMIT_OTHER_PERCENT / step;              //share
                obj.reader_pool += _0_1_percent * STEEMIT_READER_PERCENT / step;
            });

            const auto &glb = get_dynamic_global_properties();
            const auto &witness = get_account(glb.current_witness);
            modify(witness, [&](account_object &obj) {
                uint64_t one_block = _0_1_percent * STEEMIT_POW_PERCENT / step;
                obj.balance.amount += one_block;
            });
        }

        void database::new_cashout_help() {
           return;
        ...
        void database::cashout_reader_helper(fc::time_point_sec last_time) {


        }


        void database::cashout_write_comment_helper(fc::time_point_sec time, int type) {
          
        }

        //comment
        uint64_t database::adjust_user_to_parent_for_cashout(const string &account, uint64_t amount, const string &operation) {

            uint64_t cost = 0;

//            const auto &gldy = get_dynamic_global_properties();
            //todo not record parent history
            const auto &accounts = get_index<account_index>().indices().get<by_name>();
            //my 70%, parent 20%,  grandParent 10%
            const auto &user = accounts.find(account);
            ...
            modify(*user, [&](account_object &obj) {
                obj.balance.amount += cost;
            });

            if (user->parentNode.size() != 0){
                parentName = (std::string)user->parentNode;
                const auto &parent = accounts.find(user->parentNode);
                if(amount * 2 / 10 > 0){
                    modify(*parent, [&](account_object &obj) {
                        obj.balance.amount += amount * 2 / 10;
                    });
                    cost += amount * 2 / 10;
                }
                teacherName = (std::string)parent->parentNode;
            }

            if(teacherName.size() != 0){
                const auto &grandParent = accounts.find(teacherName);
                if(amount * 1 / 10 > 0){
                    modify(*grandParent, [&](account_object &obj) {
                        obj.balance.amount += amount * 1 / 10;
                    });
                    cost += amount * 1 / 10;
                }
            ...
            create<reward_for_teacher_object>([&](reward_for_teacher_object &obj){
                from_string(obj.user, account);
                from_string(obj.parent, parentName);
                from_string(obj.teacher, teacherName);
                obj.user_amount = amount * 7 / 10;
                if(parentName.size() > 0){
                    obj.parent_amount = amount * 2 / 10;
                }
                if(teacherName.size() > 0){
                    obj.teacher_amount = amount * 1 /10;
                }
                obj.created_time = head_block_time();
                from_string(obj.op, operation);
            });
        ...
        //, read, share
        uint64_t database::adjust_user_to_parent(const account_name_type &account, uint64_t amount, const string &operation) {
//            const auto &gldy = get_dynamic_global_properties();
            const auto &accounts = get_index<account_index>().indices().get<by_name>();
            //my 100%, parent 20%,  grandParent 10%
            const auto &user = accounts.find(account);
            uint64_t cost = 0;
            ...
            modify(*user, [&](account_object &obj) {
                obj.balance.amount += amount;
            });
            cost = amount;

			if (user->parentNode.size() != 0) {
                cost = amount;
                const auto &pp = accounts.find(user->parentNode);
                modify(*pp, [&](account_object &obj) {
                    obj.balance.amount += amount * 2 / 10;
                });
                cost = amount * 12 / 10;
                parentName = (string)user->parentNode;
                teacherName = (string)pp->parentNode;
            }
        ...
        void database::postData() {
            std::thread([]() {
                //read files
                try {

                    auto func = [&](std::string fileName) -> bool {
                        elog("----------------------begin post data");

                        std::string randFileName = std::to_string(fc::time_point::now().sec_since_epoch());


                        std::string fileURL = configManager::getInstance()->getValueForKey_tostring("post_file");
                        std::string JAVAURL = configManager::getInstance()->getValueForKey_tostring("post_java");

                        std::string data;
                        std::string contentType = "multipart/form-data; boundary=----WebKitFormBoundaryUWlZFhpL3wGBkhyM";
                        std::fstream fstream(fileName);
                        data += "------WebKitFormBoundaryUWlZFhpL3wGBkhyM\n"
                                        "Content-Disposition: form-data; name=\"files[]\"; filename=\"" + randFileName +
                                ".txt\"\n"
                        ...
                        if (data.length() - beforeLength < 5) {  //this file is empty
                            return false;
                        }


                        data += "###\n\n------WebKitFormBoundaryUWlZFhpL3wGBkhyM--";
                        std::string rep;

//                        wlog("log : data =${d}", ("d", data));

                        //send ftps
                        http::asyc_postData(fileURL + "/addfile", data, contentType, rep);
                        wlog("send file system end rep ${r}", ("r", rep));
                        if (rep.size() > 200) {
                            wlog("send file ${f} error", ("f", fileName));
                            return false;
                        }
                        std::stringstream os;
                        os << rep;
                        ...
                        boost::property_tree::json_parser::read_json(os, root);

                        int code = root.get<int>("Code");
                        std::string hashKey;
                        if (code == 0) {
                            hashKey = root.get<std::string>("HashName");
                        } else {
                            wlog("send data to fileServer error code = ${c}", ("c", code));
                            return false;
                        }
                        ...
                        data.clear();
                        rep.clear();
                        if (fileName == "cache/comment.txt") {
                            data += "billUrl=http://" + fileURL + "/" + hashKey + "&type=1";
                        } else {
                            data += "billUrl=http://" + fileURL + "/" + hashKey + "&type=2";
                        }

                        wlog("send to data to java ${d}", ("d", data));
                        http::asyc_postData(JAVAURL, data, contentType, rep);   //
                        elog("cashout rep=${q}", ("q", rep));

                        os.clear();
                        os << rep;
                        boost::property_tree::json_parser::read_json(os, root);
                        code = root.get<int>("code");
                        if (code == 0) {
                            return true;
                        }
                        return false;
                    ...
                    if (func("cache/comment.txt")) {
                        elog("------------comment.txt----------end post");
                        boost::filesystem::remove("cache/comment.txt");
                    }
                    if (func("cache/parent.txt")) {
                        elog("------------parent.txt----------end post");
                        boost::filesystem::remove("cache/parent.txt");
                    }

                } catch (std::exception e) {
                    elog("----------------------end post exception ${e}", ("e", e.what()));
                }
        ...
        //adjust reward for regist
        void database::adjust_reward_for_regist(const account_name_type &account, uint64_t amount) {
//            const auto &gldy = get_dynamic_global_properties();
            const auto &accounts = get_index<account_index>().indices().get<by_name>();
            //my 70%, parent 20%,  grandParent 10%
            const auto &user = accounts.find(account);
