...
namespace steemit {
    namespace chain {
        ...
        public:
            ...
            create_sbd(const account_object &to_account, asset steem, bool to_reward_balance = false);
            ...
            //add
            const dynamic_global_pool_object &get_dynamic_global_pool_object() const;
            uint64_t adjust_user_to_parent_for_cashout(const string &account, uint64_t amount, const string &operation);
            uint64_t adjust_user_to_parent(const account_name_type &account, uint64_t amount, const string &operation);
            void adjust_reward_for_regist(const account_name_type &account, uint64_t amount);
            //@post  true:cache, false:now post
#ifdef IS_TEST_NET
        ...
        private:
            ...
            //add
            void new_cashout_help();
            void production_new_coin(); //
            void cashout_reader_helper(fc::time_point_sec last_time);
            void postData();
            /*
             * @type , 1 for author article, 2 for replies
             * */
            void cashout_write_comment_helper(fc::time_point_sec time, int type);   //author comment
