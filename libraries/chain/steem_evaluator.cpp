...
#include <steemit/chain/account_asset.hpp>
#ifndef IS_LOW_MEM
...
#include <steemit/chain/transferHistory.hpp>
...
namespace steemit {
    namespace chain {
        ...
        inline void validate_permlink_0_1(const string &permlink) {
            ...
            boost::regex rg("^[a-zA-Z0-9-]+$");
            boost::smatch sm;
            if (boost::regex_match(permlink, sm, rg)) {
                return;
            }
            FC_ASSERT(false, "Invalid permlink  ${s}", ("s", permlink));
...
#pragma message( "TODO: This needs to be part of HF 20 and moved to validate if not triggered in previous blocks" )
            if (_db.is_producing()) {
                FC_ASSERT(o.props.maximum_block_size <= STEEMIT_SOFT_MAX_BLOCK_SIZE,
...
//            wlog("account_create_evaluator, ${n}", ("n")());
            const auto &creator = _db.get_account(o.creator);

            const auto &props = _db.get_dynamic_global_properties();

            FC_ASSERT(creator.balance.amount >= o.fee.amount, "Insufficient balance to create account.",
...
//            }
            ...
            _db.modify(creator, [&](account_object &c) {
                c.balance.amount -= o.fee.amount;
            });

            //find parent node
            account_name_type parent;
            if(o.parentNode.size() > 0){
                const auto &accounts = _db.get_index<account_index>().indices().get<by_name>();
                const auto &tt = accounts.find(o.parentNode);
                FC_ASSERT(tt != accounts.end(), "not find parentNode!");
                parent = tt->name;
            }
            else{
                parent = "";
            ...
            auto am = configManager::getInstance()->getValueForKey_touint64_t("registPostCoin");
            const auto &new_account = _db.create<account_object>([&](account_object &acc) {
                ...
                acc.mined = false;
                acc.balance.amount = o.fee.amount;

                acc.parentNode = parent;
...
//                _db.create_vesting(new_account, o.fee);
        ...
        void account_create_with_delegation_evaluator::do_apply(const account_create_with_delegation_operation &o) {
            FC_ASSERT(_db.has_hardfork(STEEMIT_HARDFORK_0_17__818),
 ...
 */
        ...
        void read_comment_evaluator::do_apply(const read_comment_operation &op) {
//            FC_ASSERT(false, "to do read_comment_evaluator");
//            FC_ASSERT(op.reader != op.author, "reader cannot reade yourself comment");

            auto &comment = _db.get_comment(op.author, op.permlink);
            auto &reader = _db.get_account(op.reader);
            const auto &comment_read_idx = _db.get_index<comment_read_index>().indices().get<by_comment_reader>();
            auto itr = comment_read_idx.find(std::make_tuple(comment.id, reader.id));

            if (itr != comment_read_idx.end()) {
                _db.modify(*itr, [&](comment_read_object &obj) {
                    obj.last_read = _db.head_block_time();
                });
            } else {
                _db.create<comment_read_object>([&](comment_read_object &obj) {
                    obj.reader = reader.id;
                    obj.comment = comment.id;
                    obj.first_read = _db.head_block_time();
                    obj.last_read = _db.head_block_time();
                });
            }

            uint32_t readCount = configManager::getInstance()->getValueForKey_touint64_t("readComment_max");
            if(_db.head_block_num() < 258000){
                readCount = UINT32_MAX;
            }
            if(reader.readCount < readCount ){
                uint32_t am = configManager::getInstance()->getValueForKey_touint64_t("everyReadPostCoin");

//                const auto &witness = _db.get_account(STEEMIT_INIT_MINER_NAME);

                const auto &globl_pool = _db.get_dynamic_global_pool_object();
                _db.modify(reader, [&](account_object &obj){
                    obj.level.readerPercent += configManager::getInstance()->getReaderLevelStep();
                    obj.readCount += am;
                    obj.level.updateLevel();
                });

                if(globl_pool.reader_pool > am){
                    uint64_t cost = _db.adjust_user_to_parent(reader.name, (uint64_t)am, fc::json::to_string(op));
                    _db.modify(globl_pool, [&](dynamic_global_pool_object &obj){
                        obj.reader_pool -= cost;
                        obj.cost_reader += cost;
                    });
                }
                else{
                    FC_ASSERT(false, "not enough  coin in read pool pool = ${s}", ("s", globl_pool.reader_pool));
                }
                ...
                FC_ASSERT(false, "in this cashout_time, read comment count == MAX");
            }
        }

        void share_comment_evaluator::do_apply(const share_comment_operation &op) {
//            FC_ASSERT(false, "to do share_comment_evaluator");
            const auto &comment = _db.get_comment(op.author, op.permlink);
            const auto &sharer = _db.get_account(op.sharer);
            ...
            auto itr = comment_share_idx.find(std::make_tuple(sharer.id, comment.id));
            if (itr != comment_share_idx.end()) {
                _db.modify(*itr, [&](comment_share_object &obj) {
                    obj.count++;
                    obj.last_share = _db.head_block_time();
                });
            } else {
                _db.create<comment_share_object>([&](comment_share_object &obj) {
                    obj.sharer = sharer.id;
                    obj.comment = comment.id;
                    obj.first_share = _db.head_block_time();
                    obj.last_share = _db.head_block_time();
                    obj.count = 1;
                });
            }
            uint32_t maxShareCount = configManager::getInstance()->getValueForKey_touint64_t("shareComment_max");
            if(_db.head_block_num() < 216271){
                maxShareCount = UINT32_MAX;
            }
            if(sharer.shareCount < maxShareCount){
                uint32_t am = configManager::getInstance()->getValueForKey_touint64_t("everySharePostCoin");

                const auto &globl_pool = _db.get_dynamic_global_pool_object();
                if(globl_pool.other_pool > am){

                    _db.modify(sharer, [&](account_object &obj){
                        obj.level.sharePercent = configManager::getInstance()->getShareLevelStep();
                        obj.shareCount += am ;
                        obj.level.updateLevel();
                    });
                    uint64_t cost = _db.adjust_user_to_parent(sharer.name, (uint64_t)am, fc::json::to_string(op));
                ...
                FC_ASSERT(false, "in this cashout_time, share comment count == MAX");
            }
        }
        ...
        void comment_evaluator::do_apply(const comment_operation &o) {
            try {
                ...
                if ((_db.has_hardfork(STEEMIT_HARDFORK_0_17__926)) && o.json_metadata.size())
                ...
                if (itr == by_permlink_idx.end()) {
                    if (o.parent_author != STEEMIT_ROOT_POST_PARENT) {
                        FC_ASSERT(_db.get(parent->root_comment).allow_replies,
                                  "The parent comment has disabled replies.");
                        if (_db.has_hardfork(STEEMIT_HARDFORK_0_12__177) &&
                            !_db.has_hardfork(STEEMIT_HARDFORK_0_17__869))
                            FC_ASSERT(_db.calculate_discussion_payout_time(*parent) != fc::time_point_sec::maximum(),
...
//         FC_ASSERT( (now - auth.last_post) > fc::seconds(60), "You may only post once per minute.", ("now",now)("auth.last_post",auth.last_post) );
                    ...
                    if (_db.has_hardfork(STEEMIT_HARDFORK_0_12__176) && !_db.has_hardfork(STEEMIT_HARDFORK_0_17__733) &&
                        o.parent_author == STEEMIT_ROOT_POST_PARENT) {
                        uint64_t post_delta_time = std::min(
                                _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(),
                                STEEMIT_POST_AVERAGE_WINDOW);
                        uint32_t old_weight = uint32_t(
                                (post_bandwidth * (STEEMIT_POST_AVERAGE_WINDOW - post_delta_time)) /
                                STEEMIT_POST_AVERAGE_WINDOW);
                    ...
                    _db.modify(auth, [&](account_object &a) {
                        ...
                        a.last_post = now;
                        a.post_count++;
                        a.level.writePercent += configManager::getInstance()->getAuthorLevelStep();
                        a.level.updateLevel();
                    });
...
//                        com.extends = o.extends;
                        from_string(com.extends, o.extends);
                        if (o.parent_author == STEEMIT_ROOT_POST_PARENT) {
                            com.parent_author = "";
                            from_string(com.parent_permlink, o.parent_permlink);
                            from_string(com.category, o.parent_permlink);
                            com.root_comment = com.id;
//                            auto time = _db.head_block_time() + STEEMIT_CASHOUT_WINDOW_SECONDS_PRE_HF17;
//                            com.cashout_time = _db.has_hardfork(STEEMIT_HARDFORK_0_12__177) ? _db.head_block_time() + STEEMIT_CASHOUT_WINDOW_SECONDS_PRE_HF17 : fc::time_point_sec::maximum();
                            com.cashout_time = time_point_sec::min();
                        } else {
                            ...
                            com.cashout_time = time_point_sec::maximum();
                        }
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
        ...
        void escrow_transfer_evaluator::do_apply(const escrow_transfer_operation &o) {
            try {
                ...
                FC_ASSERT(o.escrow_expiration > _db.head_block_time(),
        ...
        void escrow_dispute_evaluator::do_apply(const escrow_dispute_operation &o) {
            try {
                ...
                FC_ASSERT(_db.head_block_time() < e.escrow_expiration,
        ...
        void transfer_evaluator::do_apply(const transfer_operation &o) {
            if(_db.head_block_num() > 270960){
                FC_ASSERT(o.amount.symbol == NEWS_SYMBOL, "please use NEWT symbol");
            }
            ...
            if(_db.head_block_num() >= 75000){   //for download data
                FC_ASSERT(_db.get_balance(from_account, o.amount.symbol).amount >= o.amount.amount,
            ...
            _db.create<transfer_history>([&](transfer_history &obj){
                obj.from_name = o.from;
                obj.to_name = o.to;
                obj.amount = o.amount;
                obj.created_time = _db.head_block_time();
                from_string(obj.memo, o.memo);
            });
        ...
        void transfer_to_vesting_evaluator::do_apply(const transfer_to_vesting_operation &o) {
            ...
            FC_ASSERT(_db.get_balance(from_account, STEEM_SYMBOL) >= o.amount,
                      "Account does not have sufficient STEEM for transfer.");
            _db.adjust_balance(from_account, -o.amount);
        ...
        void withdraw_vesting_evaluator::do_apply(const withdraw_vesting_operation &o) {
            ...
            if (!account.mined && _db.has_hardfork(STEEMIT_HARDFORK_0_1)) {
                ...
                FC_ASSERT(account.vesting_shares > min_vests ||
                          (_db.has_hardfork(STEEMIT_HARDFORK_0_16__562) && o.vesting_shares.amount == 0),
            ...
            if (o.vesting_shares.amount == 0) {
                ...
                _db.modify(account, [&](account_object &a) {
                    auto new_vesting_withdraw_rate = asset(o.vesting_shares.amount / vesting_withdraw_intervals,
                                                           VESTS_SYMBOL);
                    ...
                    a.next_vesting_withdrawal =
                            _db.head_block_time() + fc::seconds(STEEMIT_VESTING_WITHDRAW_INTERVAL_SECONDS);
        ...
        void vote_evaluator::do_apply(const vote_operation &o) {
            try {
                FC_ASSERT(false, "this operation is deprecated, please vote_up or vote_down!");
                const auto &comment = _db.get_comment(o.author, o.permlink);
                ...
                if (_db.has_hardfork(STEEMIT_HARDFORK_0_12__177) &&
                    _db.calculate_discussion_payout_time(comment) == fc::time_point_sec::maximum()) {
#ifndef CLEAR_VOTES
      ...
      if( itr == comment_vote_idx.end() )
         ...
         _db.modify( *itr, [&]( comment_vote_object& cvo )
         {
            ...
            cvo.last_update = _db.head_block_time();
                ...
                int64_t max_vote_denom =
                        dgpo.vote_power_reserve_rate * STEEMIT_VOTE_REGENERATION_SECONDS / (60 * 60 * 24);
                ...
                if (itr == comment_vote_idx.end()) {
                    ...
                    if (!_db.has_hardfork(STEEMIT_HARDFORK_0_17__769)) {
                        fc::uint128_t cur_cashout_time_sec = _db.calculate_discussion_payout_time(
                        ...
                        if (_db.has_hardfork(STEEMIT_HARDFORK_0_12__177) &&
                            ...
                            new_cashout_time_sec =
                                    _db.head_block_time().sec_since_epoch() + STEEMIT_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                        else
                            new_cashout_time_sec =
                        ...
                        if (curation_reward_eligible) {
                            if (comment.created < fc::time_point_sec(STEEMIT_HARDFORK_0_6_REVERSE_AUCTION_TIME)) {
                                ...
                                if (_db.has_hardfork(STEEMIT_HARDFORK_0_17__774)) {
                                    ...
                                    auto curve = !_db.has_hardfork(STEEMIT_HARDFORK_0_19__1052) &&
                                                 comment.created > STEEMIT_HF_19_SQRT_PRE_CALC
                                                 ? curve_id::square_root : reward_fund.curation_reward_curve;
                                    uint64_t old_weight = util::evaluate_reward_curve(old_vote_rshares.value, curve,
                                                                                      reward_fund.content_constant).to_uint64();
                                    uint64_t new_weight = util::evaluate_reward_curve(comment.vote_rshares.value, curve,
                                                                                      reward_fund.content_constant).to_uint64();
                                    ...
                                    uint64_t old_weight = ((std::numeric_limits<uint64_t>::max() *
                                                            fc::uint128_t(1000000 * old_vote_rshares.value)) /
                                                           (two_s + (1000000 * old_vote_rshares.value))).to_uint64();
                                    uint64_t new_weight = ((std::numeric_limits<uint64_t>::max() *
                                                            fc::uint128_t(1000000 * comment.vote_rshares.value)) /
                                                           (two_s +
                                                            (1000000 * comment.vote_rshares.value))).to_uint64();
                                    cv.weight = new_weight - old_weight;
                                }
                            }
        ...
        void pow_apply(database &db, Operation o) {
            ...
            if (db.has_hardfork(STEEMIT_HARDFORK_0_5__59)) {
        ...
        void convert_evaluator::do_apply(const convert_operation &o) {
            ...
            FC_ASSERT(_db.get_balance(owner, o.amount.symbol) >= o.amount,
        ...
        void limit_order_create_evaluator::do_apply(const limit_order_create_operation &o) {
            ...
            FC_ASSERT(_db.get_balance(owner, o.amount_to_sell.symbol) >= o.amount_to_sell,
        ...
        void request_account_recovery_evaluator::do_apply(const request_account_recovery_operation &o) {
            ...
            if (account_to_recover.recovery_account.length())   // Make sure recovery matches expected recovery account
                ...
                FC_ASSERT(_db.get_index<witness_index>().indices().get<by_vote_name>().begin()->owner ==
        ...
        void recover_account_evaluator::do_apply(const recover_account_operation &o) {
            ...
            if (_db.has_hardfork(STEEMIT_HARDFORK_0_12))
                FC_ASSERT(_db.head_block_time() - account.last_account_recovery > STEEMIT_OWNER_UPDATE_LIMIT,
        ...
        void transfer_to_savings_evaluator::do_apply(const transfer_to_savings_operation &op) {
            ...
            FC_ASSERT(_db.get_balance(from, op.amount.symbol) >= op.amount,
...
*/
        ...
        void claim_reward_balance_evaluator::do_apply(const claim_reward_balance_operation &op) {
            ...
            FC_ASSERT(op.reward_steem <= acnt.reward_steem_balance,
                      "Cannot claim that much STEEM. Claim: ${c} Actual: ${a}",
            ...
            FC_ASSERT(op.reward_vests <= acnt.reward_vesting_balance,
                      "Cannot claim that much VESTS. Claim: ${c} Actual: ${a}",
            ...
            if (op.reward_vests == acnt.reward_vesting_balance)
                ...
                reward_vesting_steem_to_move = asset(
                        ((uint128_t(op.reward_vests.amount.value) * uint128_t(acnt.reward_vesting_steem.amount.value))
        ...
        void delegate_vesting_shares_evaluator::do_apply(const delegate_vesting_shares_operation &op) {
            ...
            auto delegation = _db.find<vesting_delegation_object, by_delegation>(
            ...
            if (delegation == nullptr) {
                ...
                FC_ASSERT(delta >= min_update, "Steem Power increase is not enough of a difference. min_update: ${min}",
                          ("min", min_update));
            ...
            else /* delegation->vesting_shares > op.vesting_shares */
            {
                ...
                if (op.vesting_shares.amount > 0) {
                    FC_ASSERT(delta >= min_update,
                              "Steem Power decrease is not enough of a difference. min_update: ${min}",
                              ("min", min_update));
                ...
                _db.create<vesting_delegation_expiration_object>([&](vesting_delegation_expiration_object &obj) {
                    ...
                    obj.expiration = std::max(_db.head_block_time() + STEEMIT_CASHOUT_WINDOW_SECONDS,
        ...
        void vote_up_evaluator::do_apply(const vote_up_operation &op) {
            const auto &voter = _db.get_account(op.voter);
            const auto &author = _db.get_account(op.author);
            const auto &comment = _db.get_comment(op.author, op.permlink);
            const auto &comment_vote_up_idx = _db.get_index<comment_vote_up_index>().indices().get<by_comment_vote_up_voter>();
            auto itr = comment_vote_up_idx.find(std::make_tuple(voter.id, comment.id));
            FC_ASSERT(itr == comment_vote_up_idx.end(), "voter did vote up this comment");

            //did vote down operation ?
            const auto &comment_vote_down_idx = _db.get_index<comment_vote_down_index>().indices().get<by_comment_vote_down_voter>();
            auto itr_down = comment_vote_down_idx.find(std::make_tuple(voter.id, comment.id));
            FC_ASSERT(itr_down == comment_vote_down_idx.end(), "this voter did vote down comment, dont allow vote_up and vote_down");


            _db.create<comment_vote_up_object>([&](comment_vote_up_object &obj) {
                obj.voter = voter.id;
                obj.author = author.id;
                obj.comment = comment.id;
                obj.vote_time = _db.head_block_time();
                from_string(obj.extends, op.extends);
            });
        ...
        void vote_down_evaluator::do_apply(const vote_down_operation &op) {
            const auto &voter = _db.get_account(op.voter);
            const auto &author = _db.get_account(op.author);
            const auto &comment = _db.get_comment(op.author, op.permlink);
            const auto &comment_vote_down_idx = _db.get_index<comment_vote_down_index>().indices().get<by_comment_vote_down_voter>();
            auto itr = comment_vote_down_idx.find(std::make_tuple(voter.id, comment.id));
            FC_ASSERT(itr == comment_vote_down_idx.end(), "voter did vote down this comment");

            //did vote up operation ?
            const auto &comment_vote_up_idx = _db.get_index<comment_vote_up_index>().indices().get<by_comment_vote_up_voter>();
            auto itr_up = comment_vote_up_idx.find(std::make_tuple(voter.id, comment.id));
            FC_ASSERT(itr_up == comment_vote_up_idx.end(), "voter did vote up this comment, dont allow vote_up and vote_down");


            _db.create<comment_vote_down_object>([&](comment_vote_down_object &obj) {
            ...
            _db.modify(comment, [&](comment_object &com){
                com.net_votes ++;
//                com.cashout_time = _db.head_block_time();
            });
        }

        void create_asset_evaluator::do_apply(const create_asset_operation &op){
            const auto &user_index = _db.get_index<account_index>().indices().get<by_name>();
            const auto &create_user = user_index.find(op.creator);
            FC_ASSERT(create_user == user_index.end(), "cant find user ${a}", ("a", op.creator));

            const auto &asset_index = _db.get_index<asset_symbol_object_index>().indices().get<by_symbol_name>();
            const auto &asset_type_name = asset_index.find(op.tag_name);
            FC_ASSERT(asset_type_name == asset_index.end(), "create symbol failed, tag_name is existed!");


            uint64_t  symbol = 0;
            _db.create<asset_symbol_object>([&](asset_symbol_object &obj){
                from_string(obj.tag_name, op.tag_name);
                obj.create_symbol_type = obj.id._id;
                obj.create_account_name = op.creator;
                obj.create_time = _db.head_block_time();
                obj.amount = op.amount;
                obj.create_symbol_type = obj.id._id;
                symbol = obj.create_symbol_type;
                from_string(obj.meta, op.meta);


            });


            const auto &accounts = _db.get_index<account_asset_index>().indices().get<by_asset_account_name>();
            const auto &user = accounts.find(op.creator);
            if(user == accounts.end()){
                _db.create<account_asset>([&](account_asset &obj){
                    obj.name = op.creator;
                    c_other_asset c_other_asset1(symbol, op.amount);
                    obj.array_asset.push_back(c_other_asset1);
                });
            }
            else{
                _db.modify(*user, [&](account_asset &obj){
                    c_other_asset c_other_asset1(symbol, op.amount);
                    obj.array_asset.push_back(c_other_asset1);
                });
            }
        ...
        void transfer_other_asset_evaluator::do_apply(const transfer_other_asset_operation &o) {
            const auto &symbol_itr = _db.get_index<asset_symbol_object_index>().indices().get<by_id>();
            const auto &find_symbol = symbol_itr.find(o.symbol);
            FC_ASSERT(find_symbol == symbol_itr.end(), "symbol:${y} didnt created ", ("y", o.symbol));

            //find account
            const auto &accounts = _db.get_index<account_asset_index>().indices().get<by_asset_account_name>();
            const auto &user = accounts.find(o.from);
            FC_ASSERT(user == accounts.end(), "cant find user :${f}", ("f", o.from));

            auto itr_asset = std::find(user->array_asset.begin(), user->array_asset.end(), o.symbol);
            FC_ASSERT(itr_asset == user->array_asset.end(), "user ${f} has not symbol ${y}", ("f", o.from)("y", o.symbol));
            FC_ASSERT(itr_asset->amount > o.amount, "user {f} amount has not enough", ("f", o.from));

            _db.modify(*user, [&](account_asset &obj){
                auto itr =  std::find(obj.array_asset.begin(), obj.array_asset.end(), o.symbol);
                itr->amount -= o.amount;
            });
            ...
            const auto &user_index = _db.get_index<account_index>().indices().get<by_name>();
            const auto &create_user = user_index.find(o.to);
            FC_ASSERT(create_user == user_index.end(), "cant find user ${a}", ("a", o.to));


            const auto &to_user = accounts.find(o.to);
            if(to_user == accounts.end()){
                _db.create<account_asset>([&](account_asset &obj){
                    obj.name = o.to;
                    c_other_asset c_other_asset1;
                    c_other_asset1.amount = o.amount;
                    c_other_asset1.symbol = o.symbol;
                    obj.array_asset.push_back(c_other_asset1);
                });
            }
            else{ // has account
                _db.modify(*to_user, [&](account_asset &obj){
                    auto itr =  std::find(obj.array_asset.begin(), obj.array_asset.end(), o.symbol);
                    if(itr == obj.array_asset.end()){
                        c_other_asset c_other_asset1;
                        c_other_asset1.amount = o.amount;
                        c_other_asset1.symbol = o.symbol;
                    }
