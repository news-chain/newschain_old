    ...
    namespace follow {
        ...
        namespace detail {
            ...
            void follow_plugin_impl::plugin_initialize() {
                ...
                _custom_operation_interpreter = std::make_shared<generic_custom_operation_interpreter<steemit::follow::follow_plugin_operation> >(
                        database());
...
//                void operator()(const read_comment_operation &op) const {
//                        int i = 0;
//                }
                ...
                void operator()(const custom_json_operation &op) const {
                    try {
                        if (op.id == FOLLOW_PLUGIN_NAME) {
                            ...
                            std::shared_ptr<custom_operation_interpreter> eval = _plugin.database().get_custom_json_evaluator(
                                    op.id);
                            eval->apply(new_cop);
                        }
                    }
